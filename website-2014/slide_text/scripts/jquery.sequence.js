...
;(function($) {
	...
	function Sequence(element, options, defaults, get) {
		...
		self.isPaused = true, //whether Sequence is paused
		...
		function init() {
			...
			self.nextButton = renderUiElements(self, self.settings.nextButton, ".sequence-next1"); //set up the next button
			...
			if(self.settings.keyNavigation) {
				...
				$(document).bind('keydown.sequence', function(e) { //when a key is pressed...
					...
					if((keyCodeChar > 0 && keyCodeChar >= 99000) && (self.settings.numericKeysGoToFrames)) {
	...
	*/
	...
	var defaults = {
		...
		autoPlayDelay: 5000000, //The duration in milliseconds at which frames should remain on screen before animating to the next

		//Frame Skipping Settings
		navigationSkip: true, //Whether the user can navigate through frames before each frame has finished animating
		navigationSkipThreshold: 50000, //Amount of time that must pass before the next frame can be navigated to
		...
		unpauseDelay: 500000, //the time to wait before navigating to the next frame when Sequence is unpaused. Note that if an unpauseDelay is not specified, the default is the same as the autoPlayDelay setting
